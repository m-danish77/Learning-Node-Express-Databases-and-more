...
const getHomeRules = [
  ...
  async (req, res, next) => {
    ...
    const home = await Home.findById(homeId);
    if (!home) {
      return res.send('<h1><center>Home Does Not Exist</center></h1>');
    }
    const filePath = home.rules;
    // __dirname points to the current directory, so we need to go up one level to get to the project root that's why we use '..'
